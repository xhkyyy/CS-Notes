...
简要总结下原理：
前提：
1）扩容的时候按照原来的倍数扩容，那么也是 2 的 N 次方。
2）当拿着 key 向桶里映射的时候是通过 h & (length-1) 映射的，其中 length 表示的是桶的大小，h 为 key 的 hash。

扩容的大小减 1 之后，高位多了 1，如：
0000 1111
扩容后
0001 1111

如果此时某个 key 的 hash 满足如下情况：
第 4 位（做左到右）为 1，那么，由于经过 h & (length-1) 运算后，值会发生变化，所以，当前这个 key 需要重新计算 hash 值。
同样，
如果某个 key 的 hash 第 4 为原来是 0，那么经过 “与” 运算后，仍然是 0，所以，这个 key 对应的桶的位置不会发生变化。
